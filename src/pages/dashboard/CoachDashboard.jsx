...
export default function CoachDashboard({ user, isAdmin }) {
    ...
    useEffect(() => {
        fetch('/api/mentorship/my_students', {
    ...
    const handleSendNote = async (e) => {
        e.preventDefault();
        try {
            await fetch('/api/mentorship/notes', {
                method: 'POST',
                headers: {
    ...
    const handleUpload = async (e) => {
        e.preventDefault();
        // For Vercel serverless, we send JSON instead of FormData
        // File uploads should be done via Supabase Storage separately
        const contentData = {
            title,
            description: desc,
            type,
            plan_min_level: planLevel,
            url: 'http://example.com' // Replace with Supabase Storage URL after uploading
        };
