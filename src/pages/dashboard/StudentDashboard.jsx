...
export default function StudentDashboard({ user }) {
    ...
    useEffect(() => {
        ...
        if (params.get('status') === 'success') {
            ...
            fetch('/api/payment/confirm_payment', {
                method: 'POST',
                headers: {
        ...
        fetch('/api/mentorship/notes?view=received', {
            headers: { Authorization: `Bearer ${token}` }
        })
            .then(res => res.json())
    ...
    const handleInviteFriend = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch('/api/users/link_friend', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
    ...
    const handleAssignCoach = async (coachId) => {
        try {
            const res = await fetch('/api/mentorship/assign_coach', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
            ...
            if (res.ok) {
                ...
                // Refresh coach
                const resCoach = await fetch('/api/mentorship/my_coach', {
                    headers: { Authorization: `Bearer ${token}` }
                });
